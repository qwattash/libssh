...
#ifdef HAVE_LIBGCRYPT
static void torture_pki_import_raw(void **state)
{
    ssh_key key;
    ...
    rc = gcry_sexp_build(&key_params, &err_offset, "(genkey (rsa (nbits 4:2048)))");
    assert_int_equal(rc, GPG_ERR_NO_ERROR);
    assert_non_null(key_params);

    rc = gcry_pk_genkey(&raw, key_params);
    assert_int_equal(rc, GPG_ERR_NO_ERROR);
    assert_non_null(raw);

    rc = ssh_pki_import_raw(raw, &key, SSH_KEYTYPE_RSA);
    assert_int_equal(rc, SSH_OK);

    gcry_sexp_release(key_params);
    gcry_sexp_release(raw);
    ssh_key_free(key);
}
...
#endif
#ifdef HAVE_LIBGCRYPT
        cmocka_unit_test(torture_pki_import_raw),
#endif /* HAVE_LIBGCRYPT */
    };
